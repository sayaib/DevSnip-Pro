...
export type VectorDbId = "qdrant" | "pinecone" | "weaviate" | "chroma" | "custom";

export interface VectorDbDescriptor {
  id: VectorDbId;
  label: string;
  /** Example base URL shown as a placeholder. */
  baseUrlHint: string;
  /** What the collection field is called for this vendor. */
  collectionLabel: string;
  requiresKey: boolean;
  docsUrl: string;
}

export const VECTOR_DBS: VectorDbDescriptor[] = [
  {
    id: "qdrant",
    label: "Qdrant",
...
export interface VectorQueryOptions {
  ...
  topK: number;
  apiKey?: string;
  /** Include the stored payload/metadata in the results. */
  includePayload?: boolean;
}

export interface BuiltVectorRequest {
  url: string;
  method: "POST";
...
export interface VectorMatch {
  id: string;
  score: number;
  /** Best-effort text pulled from the payload, used as RAG context. */
  text: string;
...
export function getVectorDb(id: VectorDbId): VectorDbDescriptor {
  const entry = VECTOR_DBS.find(db => db.id === id);
  if (!entry) throw new Error(`Unknown vector database "${id}".`);
  return entry;
}

export function buildVectorQuery(options: VectorQueryOptions): BuiltVectorRequest {
...
function payloadText(payload: Record<string, unknown>): string {
  ...
  const strings = Object.values(payload).filter((value): value is string => typeof value === "string");
  return strings.sort((a, b) => b.length - a.length)[0] ?? "";
}

export function parseVectorResponse(db: VectorDbId, payload: unknown): VectorMatch[] {
  const data = payload && typeof payload === "object" ? (payload as Record<string, any>) : undefined;
  if (!data) return [];
  ...
  switch (db) {
    case "qdrant": {
      ...
      return result.map((entry: any) => {
        const inner = (entry?.payload ?? {}) as Record<string, unknown>;
    ...
    case "pinecone": {
      ...
      return matches.map((entry: any) => {
        const inner = (entry?.metadata ?? {}) as Record<string, unknown>;
        return { id: String(entry?.id ?? ""), score: Number(entry?.score ?? 0), text: payloadText(inner), payload: inner };
      });
    }
    case "weaviate": {
      ...
      return rows.map((entry: any) => {
        ...
        return {
          ...
          text: payloadText(rest as Record<string, unknown>),
    ...
    default: {
      ...
      return rows.map((entry: any, index: number) => {
        const inner = (entry?.metadata ?? entry?.payload ?? {}) as Record<string, unknown>;
        return {
 ...
 */
export function groundingScore(answer: string, context: string): { score: number; supported: number; total: number } {
  const sentences = answer
    .split(/(?<=[.!?])\s+/)
    .map(sentence => sentence.trim())
    .filter(sentence => sentence.length > 15);
  if (!sentences.length) return { score: 0, supported: 0, total: 0 };
