import { CheckSeverity, SECRET_PATTERNS, SecurityCategory, isDocumentedExampleCredential } from "./security-analysis";

/**
 * Static rules for source, configuration and infrastructure files.
 *
 * Pure and dependency-free, so the whole rule set is unit-testable without a
 * workspace. The host layer supplies file contents; nothing here reads a file.
 *
 * The design goal is a low false-positive rate, because a scanner that cries
 * wolf gets ignored. Three mechanisms do that work:
 *
 *  - Format rules over keyword rules. `AKIA…` is an AWS key id and nothing
 *    else; `password = …` is a guess.
 *  - A placeholder and environment-reference filter, so the enormous number of
 *    `password: process.env.DB_PASSWORD` and `apiKey: "<your-key>"` lines in a
 *    normal repository never become findings.
 *  - A Shannon-entropy floor on any value reported purely on a keyword match.
 */
...
export interface StaticFinding {
  ...
  severity: Exclude<CheckSeverity, "none">;
  message: string;
  remediation: string;
  reference?: string;
  file: string;
  line: number;
  /** The matched line with any secret value masked. */
  evidence: string;
}
...
interface CodeRule {
  ...
  reference?: string;
  pattern: RegExp;
  /** Restricts the rule to these file extensions (lower case, no dot). */
  extensions?: string[];
  /** Final guard. Returning false suppresses the match. */
  accept?: (line: string, file: string) => boolean;
}
...
export function looksLikeSecretValue(rawValue: string): boolean {
  ...
  if (/^[A-Za-z_$][\w$]*(?:\.[\w$]+)*\s*\(/.test(value)) return false;
...
export function redactLine(line: string): string {
  ...
  // Quoted values.
  masked = masked.replace(/(["'`])([^"'`\n]{8,})\1/g, (_full, quote, value) =>
    `${quote}${String(value).slice(0, 3)}${"•".repeat(Math.min(12, Math.max(3, String(value).length - 3)))}${quote}`);
  // Unquoted assignments.
  masked = masked.replace(/([:=]\s*)([^\s"'`,;{}()]{12,})/g, (_full, prefix, value) =>
    `${prefix}${String(value).slice(0, 3)}${"•".repeat(Math.min(12, String(value).length - 3))}`);
 ...
 */
export function credentialLiteral(raw: string): { value: string; quoted: boolean } | undefined {
  ...
  if (/^[A-Za-z_$][\w$]*(?:\.[\w$]+)+$/.test(text)) return undefined;
  // So is an interpolation or a shell/CI expression.
  if (/^[$%!&](?:\{|\(|[A-Za-z_])/.test(text) || text.includes("${")) return undefined;
...
export const CODE_RULES: CodeRule[] = [
  /* ---- Secrets -------------------------------------------------- */
  {
    ...
    accept: (line, file) => {
      ...
      if (!literal.quoted && !DATA_FILE_EXTENSIONS.has(extensionOf(file))) return false;
...
export const CLOUD_RULES: CodeRule[] = [
  {
  ...
  },
  {
    ...
    accept: line => {
      ...
      if (/\$\{|\$\(|!Ref|!Sub|!GetAtt|valueFrom|secretKeyRef|configMapKeyRef|vault:|sops:|aws_secretsmanager|data\.\w+/i.test(line)) return false;
      const literal = credentialLiteral(match[2]);
      if (!literal) return false;
      return looksLikeSecretValue(literal.value);
    }
...
const SECRET_FORMAT_RULES: CodeRule[] = SECRET_PATTERNS
  ...
  .map(pattern => ({
    rule: pattern.id,
    title: pattern.label,
    category: "configuration" as SecurityCategory,
    severity: (pattern.severity === "none" ? "low" : pattern.severity) as Exclude<CheckSeverity, "none">,
    ...
    accept: line => !isDocumentedExampleCredential(line)
  }));

export const SECRET_RULES: CodeRule[] = [...SECRET_FORMAT_RULES, ...CODE_RULES];
 ...
 */
export function scanText(file: string, text: string, rules: CodeRule[]): StaticFinding[] {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    for (const rule of applicable) {
      ...
      if (comment && rule.category !== "configuration") continue;
      // Reset lastIndex defensively in case a rule ever carries /g.
      rule.pattern.lastIndex = 0;
      if (!rule.pattern.test(line)) continue;
      if (rule.accept && !rule.accept(line, file)) continue;
      const seen = perRule.get(rule.rule) || 0;
      if (seen >= MAX_PER_RULE_PER_FILE) continue;
...
export function scanSourceText(file: string, text: string): StaticFinding[] {
  return scanText(file, text, SECRET_RULES);
}

export function scanCloudText(file: string, text: string): StaticFinding[] {
  return scanText(file, text, CLOUD_RULES);
}

/* ------------------------------------------------------------------ *
 * Dependency and security-configuration posture
 * ------------------------------------------------------------------ */
...
/** The workspace files the posture checks need, keyed by relative path. */
export type WorkspaceFiles = Map<string, string>;

function findFile(files: WorkspaceFiles, matcher: RegExp): [string, string] | undefined {
  for (const [path, text] of files) {
    if (matcher.test(path.replace(/\\/g, "/"))) return [path, text];
  }
  return undefined;
}

/** Packages that are unmaintained or carry a known, unfixable security issue. */
const DISCOURAGED_PACKAGES: Record<string, string> = {
 ...
 */
export function analyzeWorkspacePosture(files: WorkspaceFiles): PostureCheck[] {
  const checks: PostureCheck[] = [];
  const paths = [...files.keys()].map(path => path.replace(/\\/g, "/"));
  const has = (matcher: RegExp) => paths.some(path => matcher.test(path));
  ...
  if (manifest) {
    ...
    if (parsed) {
      ...
      const dangerousScripts = Object.entries(scripts).filter(([, value]) =>
