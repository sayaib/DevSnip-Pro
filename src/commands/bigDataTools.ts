...
export function registerBigDataToolsCommands(context: vscode.ExtensionContext) {
    const hubCmd = registerTrackedCommand('sayaib.hue-console.bigDataHub', () => {
        const { panel, created } = openToolPanel('bigDataHub', 'DevSnip Pro - Big Data & Analytics Tools', { enableScripts: true });
        if (!created) return;
    ...
    const schemaViewerCmd = registerTrackedCommand('sayaib.hue-console.schemaViewer', () => {
        const { panel, created } = openToolPanel('schemaViewer', 'Parquet/Avro/JSON Schema Viewer', { enableScripts: true });
        if (!created) return;
        panel.webview.html = getSchemaViewerHtml(getNonce());
    });

    const sparkSqlFormatterCmd = registerTrackedCommand('sayaib.hue-console.sparkSqlFormatter', () => {
        const { panel, created } = openToolPanel('sparkSqlFormatter', 'Spark SQL / Presto / Trino Formatter', { enableScripts: true });
        if (!created) return;
        panel.webview.html = getSparkSqlFormatterHtml(getNonce());
    });

    const dataQualityCheckerCmd = registerTrackedCommand('sayaib.hue-console.dataQualityChecker', () => {
        const { panel, created } = openToolPanel('dataQualityChecker', 'CSV/JSON Data Quality Checker', { enableScripts: true });
        if (!created) return;
        panel.webview.html = getDataQualityCheckerHtml(getNonce());
    });

    const schemaDiffCmd = registerTrackedCommand('sayaib.hue-console.schemaDiff', () => {
        const { panel, created } = openToolPanel('schemaDiff', 'Schema Diff Tool', { enableScripts: true });
        if (!created) return;
        panel.webview.html = getSchemaDiffHtml(getNonce());
    });

    const partitionCalcCmd = registerTrackedCommand('sayaib.hue-console.partitionCalc', () => {
        const { panel, created } = openToolPanel('partitionCalc', 'Data Partition Calculator', { enableScripts: true });
        if (!created) return;
        panel.webview.html = getPartitionCalcHtml(getNonce());
    });

    const deltaLakeAnalyzerCmd = registerTrackedCommand('sayaib.hue-console.deltaLakeAnalyzer', () => {
        const { panel, created } = openToolPanel('deltaLakeAnalyzer', 'Delta Lake Log & Transaction Analyzer', { enableScripts: true });
        if (!created) return;
        panel.webview.html = getDeltaLakeAnalyzerHtml(getNonce());
    });

    const sparkCostEstimatorCmd = registerTrackedCommand('sayaib.hue-console.sparkCostEstimator', () => {
        const { panel, created } = openToolPanel('sparkCostEstimator', 'Spark Cluster & Cost Estimator', { enableScripts: true });
        if (!created) return;
