...
/**
 * The security audits moved to `securityTools`, where they share the rule
 * engine with the endpoint scanner. The scan functions are re-exported here so
 * existing importers keep working against the single implementation.
 */
...
export function registerPlatformToolsCommands(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(devops, mlops, observability, observabilityStarter);
}
