...
import { UTILITY_CSS, getNonce, safePostMessage, toastScript } from "../utils/webview-ui";
...
const SHARED_CSS = UTILITY_CSS;


export function registerAdvancedToolsCommands(context: vscode.ExtensionContext) {
    const advancedToolsHubCommand = registerTrackedCommand('sayaib.hue-console.advancedToolsHub', () => {
        const { panel, created } = openToolPanel('advancedToolsHub', 'DevSnip Pro - Developer Tools', { enableScripts: true });
        if (!created) return;
    ...
    const regexBuilderCommand = registerTrackedCommand('sayaib.hue-console.regexBuilder', () => {
        const { panel, created } = openToolPanel('regexBuilder', 'Regex Builder & Tester', { enableScripts: true });
        if (!created) return;
    ...
    const jsonFormatterCommand = registerTrackedCommand('sayaib.hue-console.jsonFormatter', () => {
        const { panel, created } = openToolPanel('jsonFormatter', 'JSON/XML Formatter', { enableScripts: true });
        if (!created) return;
        const scriptUri = panel.webview.asWebviewUri(
            vscode.Uri.file(path.join(context.extensionPath, 'media', 'json-xml-formatter.js'))
        );
        ...
        const readySubscription = panel.webview.onDidReceiveMessage(message => {
            if (message?.command === 'ready' && text) {
                safePostMessage(panel, { command: 'prefill', text });
            }
        });
        panel.onDidDispose(() => readySubscription.dispose());
    });

    const hashGeneratorCommand = registerTrackedCommand('sayaib.hue-console.hashGenerator', () => {
        const { panel, created } = openToolPanel('hashGenerator', 'Hash Generator', { enableScripts: true });
        if (!created) return;
        panel.webview.html = getHashGeneratorHtml(panel.webview.cspSource, getNonce());
    });

    const base64EncoderCommand = registerTrackedCommand('sayaib.hue-console.base64Encoder', () => {
        const { panel, created } = openToolPanel('base64Encoder', 'Base64 Encoder/Decoder', { enableScripts: true });
        if (!created) return;
        panel.webview.html = getBase64EncoderHtml(panel.webview.cspSource, getNonce());
    });

    const urlEncoderCommand = registerTrackedCommand('sayaib.hue-console.urlEncoder', () => {
        const { panel, created } = openToolPanel('urlEncoder', 'URL Encoder/Decoder', { enableScripts: true });
        if (!created) return;
        panel.webview.html = getUrlEncoderHtml(panel.webview.cspSource, getNonce());
    });

    const timestampConverterCommand = registerTrackedCommand('sayaib.hue-console.timestampConverter', () => {
        const { panel, created } = openToolPanel('timestampConverter', 'Timestamp Converter', { enableScripts: true });
        if (!created) return;
        panel.webview.html = getTimestampConverterHtml(panel.webview.cspSource, getNonce());
    });

    const jsonToToonCommand = registerTrackedCommand('sayaib.hue-console.jsonToToon', () => {
        const { panel, created } = openToolPanel('jsonToToon', 'JSON \u2192 TOON Converter', { enableScripts: true });
        if (!created) return;
        const scriptUri = panel.webview.asWebviewUri(
            vscode.Uri.file(path.join(context.extensionPath, 'media', 'json-to-toon.js'))
        );
        panel.webview.html = getJsonToToonHtml(panel.webview.cspSource, String(scriptUri));
    });

    const colorPaletteCommand = registerTrackedCommand('sayaib.hue-console.colorPalette', () => {
        const { panel, created } = openToolPanel('colorPalette', 'Color Palette', { enableScripts: true });
        if (!created) return;
        const nonce = getNonce();
        panel.webview.html = getColorPaletteHtml(panel.webview.cspSource, nonce);
    });

    const loremGeneratorCommand = registerTrackedCommand('sayaib.hue-console.loremGenerator', () => {
        const { panel, created } = openToolPanel('loremGenerator', 'Lorem Ipsum Generator', { enableScripts: true });
        if (!created) return;
        const nonce = getNonce();
    ...
    context.subscriptions.push(
        advancedToolsHubCommand, regexBuilderCommand, jsonFormatterCommand,
        hashGeneratorCommand, base64EncoderCommand, urlEncoderCommand,
        timestampConverterCommand, jsonToToonCommand, colorPaletteCommand, loremGeneratorCommand
    );
}

function getAdvancedToolsHubHtml(): string {
    const nonce = getNonce();
    return `<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <script nonce="${nonce}">
        ...
        order.forEach(function(section) {
            ...
            groups[section].forEach(function(t) {
                var card = document.createElement('div'); card.className = 'hub-card'; card.setAttribute('data-command', t.cmd);
...
function getJsonFormatterHtml(cspSource: string, scriptSrc: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function getHashGeneratorHtml(cspSource: string, nonce: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function getBase64EncoderHtml(cspSource: string, nonce: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function getUrlEncoderHtml(cspSource: string, nonce: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function getTimestampConverterHtml(cspSource: string, nonce: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function getJsonToToonHtml(cspSource: string, scriptSrc: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function getColorPaletteHtml(cspSource: string, nonce: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function getLoremGeneratorHtml(cspSource: string, nonce: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
