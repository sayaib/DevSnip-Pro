...
const mockContext: vscode.ExtensionContext = createMockContext();

/** Spins up a lightweight local HTTP server for integration-style tests (no external network). */
function startTestServer(handler: (req: http.IncomingMessage, res: http.ServerResponse, body: string) => void): Promise<{ server: http.Server; url: string }> {
...
suite('API Tester - cURL Export/Import', () => {
    ...
    setup(() => {
        apiTester = new ApiTester(createMockContext());
    });
    ...
    test('parseCurlCommand extracts method, url, headers, body and basic auth', () => {
        const parsed = apiTester.parseCurlCommand(
            `curl -X POST 'https://api.example.com/login' -H 'Content-Type: application/json' -d '{"user":"bob"}' -u admin:pw123`
        );
...
suite('API Tester - Body Type Handling (via cURL generation)', () => {
    let apiTester: ApiTester;
    setup(() => { apiTester = new ApiTester(createMockContext()); });
