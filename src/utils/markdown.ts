...
function renderInline(text: string): string {
  ...
  let working = escape(text).replace(/`([^`]+)`/g, (_match, code: string) => {
    codeSpans.push(`<code>${code}</code>`);
    return `${CODE_PLACEHOLDER_PREFIX}${codeSpans.length - 1}${CODE_PLACEHOLDER_SUFFIX}`;
  });

  working = working.replace(/!\[([^\]]*)\]\(([^)\s]+)(?:\s+&quot;[^&]*&quot;)?\)/g, (match, alt: string, url: string) => {
    const safe = safeUrl(url);
    return safe ? `<img src="${safe}" alt="${alt}">` : match;
  });

  working = working.replace(/\[([^\]]+)\]\(([^)\s]+)(?:\s+&quot;[^&]*&quot;)?\)/g, (match, label: string, url: string) => {
    const safe = safeUrl(url);
  ...
  return working.replace(placeholder, (_match, index: string) => codeSpans[Number(index)] ?? "");
}
