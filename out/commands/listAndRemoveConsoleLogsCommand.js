...
function registerListAndRemoveConsoleLogsCommand(context) {
    const command = vscode.commands.registerCommand("sayaib.hue-console.listAndRemoveConsoleLogs", () => __awaiter(this, void 0, void 0, function* () {
        ...
        panel.webview.html = generateWebviewContentConsoleLoading("Analyzing your project to retrieve all console logs. Please wait...");
        for (const folder of workspaceFolders) {
            const files = yield vscode.workspace.findFiles(new vscode.RelativePattern(folder, "**/*.{ts,tsx,js,jsx}"), "**/node_modules/**");
        ...
        if (allConsoleLogs.length === 0) {
            panel.webview.html = generateWebviewContentConsoleLoading("Great job! No `console.log` statements were found in your project. You've successfully optimized your project's performance!");
        ...
        panel.webview.onDidReceiveMessage((message) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (message.command === "removeSelectedLogs") {
                yield removeSelectedLogs(message.selectedLogs);
            }
            else if (message.command === "removeAllLogs") {
                console.log(allConsoleLogs);
                yield removeSelectedLogs(allConsoleLogs);
            }
...
function removeSelectedLogs(selectedLogs) {
    return __awaiter(this, void 0, void 0, function* () {
        for (const log of selectedLogs) {
            try {
                const document = yield vscode.workspace.openTextDocument(log.filePath);
...
<body>
    ...
    <div class="container">
        ...
        .map((log) => `
              <tr>
                  <td><input type="checkbox" name="log" value="${log.lineNumber}" data-log='${JSON.stringify(log)}' /></td>
                  <td>${log.filePath}</td>
