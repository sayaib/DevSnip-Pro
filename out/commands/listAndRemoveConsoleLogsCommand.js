...
function registerListAndRemoveConsoleLogsCommand(context) {
    const command = vscode.commands.registerCommand("sayaib.hue-console.listAndRemoveConsoleLogs", () => __awaiter(this, void 0, void 0, function* () {
        ...
        const panel = vscode.window.createWebviewPanel("listConsoleLogs", "Console Log Cleaner", vscode.ViewColumn.One, { enableScripts: true });
        ...
        const files = yield vscode.workspace.findFiles(new vscode.RelativePattern(vscode.workspace.workspaceFolders[0], SEARCH_PATTERN), EXCLUDE_PATTERN);
        const scanResults = yield Promise.all(files.map((file) => scanFile(file)));
        ...
        panel.webview.onDidReceiveMessage((message) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (message.command === "removeSelectedLogs") {
                    currentConsoleLogs = yield removeSelectedLogs(message.selectedLogs, panel);
                }
                else if (message.command === "removeAllLogs") {
                    currentConsoleLogs = yield removeSelectedLogs(currentConsoleLogs, panel);
                }
                else if (message.command === "refreshLogs") {
                    currentConsoleLogs = yield fetchConsoleLogs();
...
function fetchConsoleLogs() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const files = yield vscode.workspace.findFiles(new vscode.RelativePattern(vscode.workspace.workspaceFolders[0], SEARCH_PATTERN), EXCLUDE_PATTERN);
        const results = yield Promise.all(files.map((file) => scanFile(file)));
        return results.flat();
    });
}
function scanFile(file) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            return findConsoleLogs(yield fs.readFile(file.fsPath, "utf8"), file.fsPath);
        }
        catch (error) {
...
function generateLegacyWebviewContentConsoleLoading(message) {
    const nonce = getNonce();
    return `
<!DOCTYPE html>
<html lang="en">
...
function generateLegacyWebviewContentConsole(consoleLogs) {
    const nonce = getNonce();
    return `
<!DOCTYPE html>
<html lang="en">
