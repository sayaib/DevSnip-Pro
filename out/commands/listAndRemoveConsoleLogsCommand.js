...
function registerListAndRemoveConsoleLogsCommand(context) {
    const command = (0, command_registry_1.registerTrackedCommand)("sayaib.hue-console.listAndRemoveConsoleLogs", async () => {
        if (!vscode.workspace.workspaceFolders) {
            vscode.window.showErrorMessage("No workspace is open.");
            return;
        }
        const { panel, created } = (0, webview_ui_1.openToolPanel)("listConsoleLogs", "Console Log Cleaner", { enableScripts: true });
        if (!created)
            return;
            ...
            try {
                if (message?.command === "removeSelectedLogs") {
                    ...
                    const needsConfirmation = vscode.workspace
                        ...
                        .get("consoleLogCleanup.confirmBeforeDelete", true);
                    const confirmed = !needsConfirmation || await (0, webview_ui_1.confirmAction)(`Remove all ${currentConsoleLogs.length} console.log statement(s) from your workspace?`, "Remove all");
                    if (confirmed) {
                ...
                vscode.window.showErrorMessage(`Console log cleanup failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            finally {
 ...
 */
function matchKnownLogs(selection, known) {
...
async function removeSelectedLogs(selectedLogs, panel) {
    ...
    if (stale) {
        vscode.window.showWarningMessage(`${stale} console.log statement(s) changed on disk since the scan and were skipped. Refreshing the list.`);
    }
...
function findConsoleLogs(content, filePath) {
    ...
    const isIdent = (value) => !!value && /[\w$]/.test(value);
    let i = 0;
    while (i < content.length) {
        ...
        if (content.startsWith("console", i) && !isIdent(content[i - 1])) {
            ...
            while (/\s/.test(content[cursor] || ""))
                cursor++;
            if (!content.startsWith("log", cursor) || isIdent(content[cursor + 3])) {
                i++;
                continue;
            }
            cursor += 3;
...
exports.findConsoleLogs = findConsoleLogs;
...
function findCallEnd(content, open) {
    let depth = 0;
...
<html lang="en"><head>
  ...
  <div class="table-wrap"><table id="consoleTable"><thead><tr><th class="check"><input id="selectAll" type="checkbox" aria-label="Select all visible logs"></th><th>File</th><th>Line</th><th>Log statement</th><th>Action</th></tr></thead><tbody>
    ${consoleLogs.map((log, index) => `<tr><td class="check"><input class="row-check" type="checkbox" data-index="${index}" aria-label="Select log in ${(0, webview_ui_1.escapeHtml)(log.filePath)} at line ${log.lineNumber}"></td><td class="file" title="${(0, webview_ui_1.escapeHtml)(log.filePath)}">${(0, webview_ui_1.escapeHtml)(vscode.workspace.asRelativePath(log.filePath))}</td><td class="line">${log.lineNumber}</td><td><pre>${(0, webview_ui_1.escapeHtml)(log.text)}</pre></td><td><button class="remove-log-btn danger" data-index="${index}" type="button">Remove</button></td></tr>`).join("")}
  </tbody></table></div>
</main><script nonce="${nonce}">
  const vscode = acquireVsCodeApi(); const logs = ${(0, webview_ui_1.embedJson)(consoleLogs)}; const input = document.getElementById('searchInput'); const rows = [...document.querySelectorAll('#consoleTable tbody tr')]; const checks = [...document.querySelectorAll('.row-check')]; const selectAll = document.getElementById('selectAll'); const removeSelected = document.getElementById('removeSelectedBtn'); const count = document.getElementById('resultCount');
