...
async function findReadmeFiles() {
    const found = await vscode.workspace.findFiles("**/{README,readme,Readme}.{md,markdown,MD}", "{**/node_modules/**,**/.git/**,**/dist/**,**/build/**,**/out/**}", 50);
...
function registerReadmeManagerCommand(context) {
    ...
    const command = (0, command_registry_1.registerTrackedCommand)("sayaib.hue-console.readmeManager", async () => {
        ...
        if (existing.length === 0) {
            if (!(await (0, webview_ui_1.confirmAction)("No README was found in this workspace. Create one now?", "Create README.md", "information")))
                return;
            ...
            const picked = await vscode.window.showQuickPick(existing.map(uri => ({ label: vscode.workspace.asRelativePath(uri), uri })), { title: "Which README do you want to manage?" });
            if (!picked)
                return;
            target = picked.uri;
        }
        ...
        activePanel?.dispose();
        const panel = vscode.window.createWebviewPanel("readmePreview", `README Preview - ${path.basename(target.fsPath)}`, vscode.ViewColumn.Two, { enableScripts: true, retainContextWhenHidden: true });
        activePanel = panel;
        ...
        const render = async (status) => {
            try {
                ...
                (0, webview_ui_1.safePostMessage)(panel, { command: "update", html: (0, markdown_1.renderMarkdown)(text), ...documentStats(text), status });
            }
            catch (error) {
                (0, webview_ui_1.safePostMessage)(panel, {
                    command: "update",
        ...
        const messageSubscription = panel.webview.onDidReceiveMessage(async (message) => {
            try {
                switch (message?.command) {
                    ...
                    case "delete": {
                        const confirmed = await (0, webview_ui_1.confirmAction)(`Delete ${vscode.workspace.asRelativePath(target)}? It is moved to the trash and can be restored from there.`, "Delete README");
                        if (!confirmed)
                            break;
                    ...
                    case "reset": {
                        const confirmed = await (0, webview_ui_1.confirmAction)("Replace the README contents with the starter template? The current contents will be overwritten.", "Replace contents");
                        if (!confirmed)
                            break;
                        await vscode.workspace.fs.writeFile(target, Buffer.from(README_TEMPLATE, "utf8"));
 ...
 */
function getPreviewHtml(relativePath, sections) {
    const nonce = (0, webview_ui_1.getNonce)();
    const sectionOptions = sections.map(name => `<option value="${(0, webview_ui_1.escapeHtml)(name)}">${(0, webview_ui_1.escapeHtml)(name)}</option>`).join("");
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>
  <div class="toolbar">
    <span class="path">${(0, webview_ui_1.escapeHtml)(relativePath)}</span>
