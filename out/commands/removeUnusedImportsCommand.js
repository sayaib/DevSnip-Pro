...
exports.analyzeFileImports = exports.registerRemoveUnusedImportsCommand = void 0;
...
function registerRemoveUnusedImportsCommand(context) {
    context.subscriptions.push((0, command_registry_1.registerTrackedCommand)("sayaib.hue-console.removeUnusedImports", async () => {
        const folder = vscode.workspace.workspaceFolders?.[0];
        if (!folder) {
            vscode.window.showErrorMessage("Open a workspace before checking imports.");
            return;
        }
        const { panel, created } = (0, webview_ui_1.openToolPanel)("removeUnusedImports", "Unused Import Cleaner", { enableScripts: true });
        if (!created)
            return;
            ...
            try {
                ...
                if (message?.command === "removeSelectedImports" || message?.command === "removeAllImports") {
                    if (message.command === "removeAllImports") {
                        ...
                        if (!(await (0, webview_ui_1.confirmAction)(`Remove all ${imports.length} unused import statement(s)?`, "Remove all")))
                            return;
                    }
...
async function findUnusedImports(folder) {
...
function importedSymbols(line, language) {
    ...
    if (language === ".java") {
        ...
        return symbol && !match?.[1].endsWith(".*") ? [symbol] : [];
    }
    if (language === ".py")
...
async function removeImports(imports) {
    const edit = new vscode.WorkspaceEdit();
    const documents = new Map();
    for (const item of imports) {
        const document = documents.get(item.filePath) || await vscode.workspace.openTextDocument(vscode.Uri.file(item.filePath));
