...
function maxScanFiles() {
    ...
    return Number.isFinite(configured) && configured > 0
        ? Math.min(Math.trunc(configured), 20000)
        : DEFAULT_MAX_FILES;
}
...
function toFinding(finding, resource) {
    return {
...
async function scanWith(globs, accept, rules, token) {
...
async function scanWorkspaceForSecurity(token) {
    return scanWith([SOURCE_GLOB, EXTRA_GLOB], uri => SOURCE_EXTENSIONS.has(extensionOf(uri)), security_static_1.scanSourceText, token);
}
exports.scanWorkspaceForSecurity = scanWorkspaceForSecurity;
/** Infrastructure-as-code and container rules over the workspace. */
async function scanLocalCloudConfiguration(token) {
    return scanWith([CLOUD_GLOB, EXTRA_GLOB], uri => CLOUD_EXTENSIONS.has(extensionOf(uri)), security_static_1.scanCloudText, token);
}
...
exports.collectManifestFiles = collectManifestFiles;
async function scanWorkspacePosture() {
    return (0, security_static_1.analyzeWorkspacePosture)(await collectManifestFiles());
}
exports.scanWorkspacePosture = scanWorkspacePosture;
/* ------------------------------------------------------------------ *
 * Report assembly
 * ------------------------------------------------------------------ */
/** Findings from the file scanners, in the shared check shape. */
function findingsToChecks(findings) {
...
function postureToChecks(posture) {
...
function buildReport(target, checks, startedMs, notes = []) {
    const sorted = (0, security_analysis_1.sortChecks)(checks);
    return {
        target,
        startedAt: new Date(startedMs).toISOString(),
        durationMs: Date.now() - startedMs,
        checks: sorted,
        summary: (0, security_analysis_1.summarize)(sorted),
        notes
    };
}
/** A pass entry so an empty file scan is reported as a result, not a blank. */
function cleanScanCheck(id, title, detail) {
...
function reportToMarkdown(scope, report) {
    ...
    for (const check of report.checks) {
        ...
        lines.push("");
        lines.push(`- **Area:** ${security_analysis_1.CATEGORY_LABELS[check.category] || check.category}`);
        if (check.target)
            lines.push(`- **Affected:** \`${check.target}\``);
        lines.push("");
        ...
        if (check.reference)
            lines.push("", `**Reference:** ${check.reference}`);
        lines.push("");
    }
    return lines.join("\n");
}
...
`;
function securityHubHtml(nonce, initial) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
    ...
    <div class="card">
      ...
      <div class="grid">
        <div>
          ...
          <input type="text" id="epUrl" placeholder="https://api.example.com/v1/users" value="${(0, webview_ui_1.escapeHtml)(initial.url)}" spellcheck="false">
        </div>
        <div>
...
function toWire(checks, resources) {
    return checks.map(check => ({
        ...check,
        categoryLabel: security_analysis_1.CATEGORY_LABELS[check.category] || check.category,
...
async function openLocation(file, line, resource) {
...
async function saveReport(scope, report, format) {
...
function hubDefaults(context) {
    const settings = vscode.workspace.getConfiguration("devsnip");
    const remembered = context.globalState.get("devsnip.security.lastUrl");
    const timeout = settings.get("security.endpointTimeout", 15000);
    return {
        url: typeof remembered === "string" ? remembered : "",
        timeoutMs: Number.isFinite(timeout) ? Math.min(120000, Math.max(1000, Math.trunc(timeout))) : 15000,
        activeChecks: settings.get("security.activeChecks", false) === true
    };
}
function registerSecurityToolsCommands(context) {
    const scans = new Map();
    ...
    const openHub = (focusScope, run = false) => {
        const { panel, created } = (0, webview_ui_1.openToolPanel)("devsnipSecurityHub", "DevSnip Pro · Security", {
            enableScripts: true,
        ...
        if (created) {
            ...
            const subscription = panel.webview.onDidReceiveMessage(message => {
                if (message?.type === "ready") {
                    ready.add(panel);
                    for (const queued of pending.get(panel) || [])
                        (0, webview_ui_1.safePostMessage)(panel, queued);
        ...
        if (focusScope)
            send(panel, { type: "command", scope: focusScope, run });
        return panel;
    };
    async function handleMessage(panel, message) {
        ...
        if (message.type === "cancel") {
            const state = scans.get(message.scope);
            state?.cancel?.cancel();
            return;
        }
        ...
        if (message.type === "scanEndpoint") {
            await runEndpoint(panel, message);
            return;
        }
        if (message.type === "scanWorkspace") {
            await runFileScan(panel, "workspace");
            return;
        }
        if (message.type === "scanCloud") {
            await runFileScan(panel, "cloud");
            return;
        }
        if (message.type === "scanPosture") {
            await runPosture(panel);
            return;
        }
    }
    async function runEndpoint(panel, message) {
    ...
    async function runFileScan(panel, scope) {
        ...
        try {
            ...
            for (const finding of findings) {
                if (finding.resource)
                    resources.set(`${finding.file}:${finding.line}`, finding.resource);
            }
            const checks = findingsToChecks(findings);
            if (!checks.length) {
                checks.push(cleanScanCheck(`${scope}.clean`, scope === "workspace" ? "No matching risk found in workspace source" : "No matching risk found in infrastructure files", scope === "workspace"
    ...
    async function runPosture(panel) {
        if (!vscode.workspace.workspaceFolders?.length) {
        ...
        try {
            ...
            if (!checks.length) {
                checks.push(cleanScanCheck("posture.none", "No dependency manifest found", "No package.json, requirements.txt or equivalent manifest was found in this workspace, so there was nothing for the dependency checks to read."));
            }
    ...
    const hub = (0, command_registry_1.registerTrackedCommand)("sayaib.hue-console.securityHub", () => { openHub(); });
    const endpoint = (0, command_registry_1.registerTrackedCommand)("sayaib.hue-console.endpointSecurityScan", () => { openHub("endpoint"); });
    const audit = (0, command_registry_1.registerTrackedCommand)("sayaib.hue-console.securityAudit", () => { openHub("workspace", true); });
    const cloudAudit = (0, command_registry_1.registerTrackedCommand)("sayaib.hue-console.cloudSecurityAudit", () => { openHub("cloud", true); });
    const dependencyAudit = (0, command_registry_1.registerTrackedCommand)("sayaib.hue-console.dependencyAudit", () => { openHub("posture", true); });
    context.subscriptions.push(hub, endpoint, audit, cloudAudit, dependencyAudit);
}
exports.registerSecurityToolsCommands = registerSecurityToolsCommands;
