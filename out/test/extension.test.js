    ...
    test("workspace tools complete without throwing", async () => {
        ...
        for (const command of workspaceCommands) {
            ...
            const settled = await Promise.race([
                Promise.resolve(vscode.commands.executeCommand(command)).then(() => "resolved", (error) => ({ failed: `${command} threw: ${error instanceof Error ? error.message : String(error)}` })),
                new Promise(resolve => setTimeout(() => resolve("awaiting-input"), 2000))
            ]);
            await vscode.commands.executeCommand("workbench.action.closeQuickOpen");
            assert.ok(typeof settled === "string", typeof settled === "object" && settled && "failed" in settled ? String(settled.failed) : "unknown failure");
        }
    });
